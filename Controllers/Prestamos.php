...
class Prestamos extends Controller
{
    ...
    public function listar()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            if ($data[$i]['estado'] == 1) {
                ...
                <button class="btn btn-danger" type="button" onclick="btnEliminar(' . $data[$i]['id'] . ');"><i class="fa fa-trash"></i></button>
                <div/>';
            } else {
    ...
    public function registrar()
    {
        ...
        if (empty($libro) || empty($estudiante) || empty($cantidad) || empty($fecha_prestamo) || empty($fecha_devolucion)) {
            ...
            if ($verificar_cant['cantidad'] >= $cantidad) {
                ...
                if ($data > 0) {
                    $msg = array('msg' => 'Material Prestado', 'icono' => 'success', 'id' => $data);
                } else if ($data == "existe") {
                    $msg = array('msg' => 'El Material ya está prestado', 'icono' => 'warning');
                } else {
    ...
    public function entregar($id)
    {
        ...
        if ($datos == "ok") {
            $msg = array('msg' => 'Material recibido', 'icono' => 'success');
        } else {
    ...
    public function ticked($id_prestamo)
    {
        ...
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(65, 5, "Ticket de Prestamo", 0, 1, 'C');
        $pdf->Image(base_url . "Assets/img/logo.png", 55, 20, 20, 20, 'PNG');
        ...
        $pdf->Ln();
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(65, 5, "Detalle de Prestamo", 0, 1, 'C');
        $pdf->Ln();
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(10, 5, "Cant.", 0, 0, 'L');
        $pdf->Cell(55, 5, 'Descripcion', 0, 1, 'L');
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(10, 5, $prestamo['cantidad'], 0, 0, 'L');
        $pdf->Cell(55, 5, utf8_decode($prestamo['titulo']), 0, 1, 'L');
        $pdf->Ln();
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(30, 5, 'Fecha Prestamo', 0, 0, 'L');
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(30, 5, $prestamo['fecha_prestamo'], 0, 1, 'L');
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(30, 5, 'Fecha Devolucion', 0, 0, 'L');
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(30, 5, $prestamo['fecha_devolucion'], 0, 1, 'L');
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(30, 5, 'Observacion', 0, 0, 'L');
        $pdf->SetFont('Arial', '', 8);
        $pdf->MultiCell(50, 5, utf8_decode($prestamo['observacion']), 0, 'L');
        $pdf->Ln();
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(65, 5, utf8_decode('Firma'), 0, 1, 'C');
        $pdf->Ln(10);
        $pdf->Cell(30, 5, "____________________", 0, 1, 'C');
        $pdf->Output("prestamo.pdf", "I");
    }

    public function eliminar($id)
    {
        $result = $this->model->eliminarPrestamo($id);
        if ($result == "ok") {
            $msg = array('msg' => 'Prestamo eliminado con éxito', 'icono' => 'success');
        } else {
